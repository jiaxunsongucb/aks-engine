		...
		doneCheck:
			for {
				...
				case <-time.After(30 * time.Minute):
					Fail("Fet StatefulSets status time out!")
				}
			}
			...
			for _, node := range nodeList.Nodes {
				if node.Status.NodeInfo.KubeletVersion != upgradeVersion {
					Skip(fmt.Sprintf("The cluster upgrade operation has not completed. Node %s is in version %s, not in target upgrade version %s.", node.Metadata.Name, node.Status.NodeInfo.KubeletVersion, upgradeVersion))
				}
			}
			...
			for _, e := range eventList.Events {
				if e.Reason == "NodeNotSchedulable" {
				...
				} else if e.Reason == "SuccessfulAttachVolume" {
					podName := e.InvolvedObject.Name
					if _, ok := podVolumeMap[podName]; ok {
